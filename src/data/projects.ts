...
export const projects: Project[] = [
...

  {
    ...
    solution:
      "The Go service handles registration, login, email confirmation, and password reset directly: bcrypt hashing via golang.org/x/crypto, JWTs issued and verified in middleware, and a separate role middleware gating admin routes from customer ones. Data access is raw lib/pq with SQL migrations checked into the repo — no ORM, so the queries and the schema are both explicit. The frontend keeps cart state in Zustand and validates every form with Zod before it reaches the API.",
    ...
    features: [
      ...
      },
      {
        ...
        body: "Schema changes are numbered SQL files checked into the repo, run against Postgres through raw lib/pq with no ORM in between — the queries and the schema are both explicit.",
      },
    ],
